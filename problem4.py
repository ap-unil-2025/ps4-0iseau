...
def backup_file(source_filename, backup_filename):
    ...
    return save_to_json(load_from_json(source_filename), backup_filename)
...
def get_file_stats(filename):
    ...
    dict = {}
    dict['exists'] = os.path.exists(filename)
    if dict['exists'] :
        dict['size_bytes'] = os.path.getsize(filename)
        data = load_from_json(filename)
        dict['type'] = type(data).__name__
        dict['count'] = len(data)
    return dict
...
def merge_json_files(file1, file2, output_file):
    ...
    try :
        with open(output_file, 'w') as outfile:
            json.dump(load_from_json(file1) + load_from_json(file2), outfile)
        return True
    except :
        return False
...
def search_json_file(filename, key, value):
    ...
    list = [item for item in load_from_json(filename) if key in item and item[key] == value]
    return list
